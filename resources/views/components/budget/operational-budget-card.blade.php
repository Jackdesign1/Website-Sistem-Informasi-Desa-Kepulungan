...
<x-mary-card title="Operasional Desa" subtitle="Tahun: {{ $operational->year }}" shadow separator class="relative border shadow-lg">
    <div class="absolute flex flex-col gap-1.5 top-1.5 right-3">
        ...
        <x-mary-button
            icon="tabler.trash"
            wire:click="delete('{{ Crypt::encrypt($operational->id) }}')"
            wire:confirm='Anda yakin ingin menghapus data anggaran tahun {{ $operational->year }}?'
    ...
    <x-mary-collapse :name="'group'.$index" separator>
        ...
        <div class="flex flex-col flex-wrap gap-2 sm:items-center sm:justify-between sm:flex-row">
            ...
            <span class="text-sm" title="Rp {{ number_format($operational->operationalDetails->sum('value'), 2) }}">Rp {{ number_format($operational->operationalDetails->sum('value'), 2) }}</span>
        </div>
        </x-slot:heading>
        <x-slot:content>
        <x-mary-accordion>
