<x-dashboard-container>
    ...
    <x-mary-form no-separator wire:submit="edit" >
        <div class="flex gap-3 sm:gap-3 sm:flex-row">
            <div class="flex-1">
            ...
            @foreach ($detailBudgets as $index => $detail)
                <div class="flex flex-1 gap-3 px-3 pb-4 border-b md:border-b-0 md:pb-0 last:border-b-0" wire:key='{{ $index }}'>
                    ...
                    <div class="flex items-end justify-center mt-3 md:mt-0">
                        <x-mary-button class="btn-warning" icon="tabler.trash" wire:confirm='Anda yakin ingin menghapus detail anggaran {{ $detail["type"] }}?' wire:click="removeDetailBudget('{{ $index }}')" spinner="removeDetailBudget('{{ $index }}')"></x-mary-button>
                    </div>
                </div>
            @endforeach
