<x-dashboard-container>
    <x-mary-header class="!mb-5" title="Edit data pendapatan" separator progress-indicator="edit"></x-mary-header>
    ...
    <x-mary-form no-separator wire:submit="edit">
                        ...
                        <div class="flex flex-1 gap-3" wire:key='{{ $typeIndex }}'>
                            ...
                            <div class="flex items-end">
                                <x-mary-button 
                                    ...
                                    icon="tabler.trash" 
                                    x-data 
                                    @click="if (confirm('Apakah Anda yakin ingin menghapus jenis pendapatan ini?')) { $wire.removeIncomeType('{{ $typeIndex }}') }" 
                                    spinner="removeIncomeType('{{ $typeIndex }}')">
                                </x-mary-button>
                            </div>
                        </div>
                                ...
                                <div class="flex flex-1 gap-3" wire:key='{{ $typeIndex.$detailIndex }}'>
                                    ...
                                    <div class="flex items-end">
                                        <x-mary-button 
                                            class="btn-warning" 
                                            icon="tabler.trash" 
                                            @click="if (confirm('Apakah Anda yakin ingin menghapus detail pendapatan ini?')) { $wire.removeIncomeDetail('{{ $typeIndex }}', '{{ $detailIndex }}') }" 
                                            spinner="removeIncomeDetail('{{ $typeIndex }}', '{{ $detailIndex }}')" />
                                    </div>
                                </div>
                            @endforeach
        ...
        <x-slot:actions>
            ...
            <x-mary-button label="Ubah Data Pendapatan" icon="tabler.plus" type="submit" class="btn-primary" spinner="create" />
        </x-slot:actions>
    </x-mary-form>
